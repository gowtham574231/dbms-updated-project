...
export default function PrintPage() {
  ...
  const [subjectCode, setSubjectCode] = useState("SUB101");
  const [totalMarks, setTotalMarks] = useState(100);
  const [timeDate, setTimeDate] = useState("10:00 AM, 01 Jan 2025");
  ...
  return (
    <>
      ...
      <style>{`
        ...
        /* For screen preview (center A4 look) */
        .print-preview {
          width: 210mm;
          min-height: 297mm;
          padding: 20mm;
      ...
      <div className="p-6 no-print">
        <div className="mb-4 grid grid-cols-2 gap-3">
          <input className="border p-2" value={collegeName} onChange={(e) => setCollegeName(e.target.value)} />
          <input className="border p-2" value={subjectName} onChange={(e) => setSubjectName(e.target.value)} />
          <input className="border p-2" value={subjectCode} onChange={(e) => setSubjectCode(e.target.value)} />
          <input className="border p-2" value={totalMarks} onChange={(e) => setTotalMarks(Number(e.target.value))} />
          <input className="border p-2" value={timeDate} onChange={(e) => setTimeDate(e.target.value)} />
          <button
        ...
        <div className="mb-4 grid grid-cols-3 gap-3">
          ...
          <div className="flex items-end gap-2">
            ...
            <button className="bg-gray-200 p-2 rounded" onClick={() => setSelected([])}>
              Clear Selection
            </button>
          </div>
        </div>
      ...
      <div className="print-preview print-a4">
        <div className="text-center text-2xl font-bold mb-4 text-black">
          {collegeName}
        </div>

        <div className="flex justify-between mb-1 text-black">
          <div className="font-semibold">{subjectName}</div>
          <div className="font-semibold">{subjectCode}</div>
        </div>
        ...
        {printing.length === 0 ? (
          <p className="text-center text-sm">Select questions from the table above to print.</p>
        ) : (
